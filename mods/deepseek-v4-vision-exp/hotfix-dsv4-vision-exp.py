#!/usr/bin/env python3
"""Install native DeepSeek-V4-Flash-Vision-Exp image support into vLLM.

The runtime's ``DeepseekV4ForCausalLM`` is text-only. Vision-Exp ships a 32-layer
ViT + Aligner and ``<｜deepseek_image｜>`` prompt tokens. This startup patch:
...
2. Remaps DSpark draft ``ffn.gate.bias_vl`` → ``e_score_correction_bias_vl``
    (the base loader only rewrites names ending in ``.ffn.gate.bias``).
3. Relaxes the Vision-Exp encoder's rejection of already-substituted
   ...
   vLLM's chat parser, then restores the official Chat Completions rule:
    images in prompt and tool-result messages (``system`` / ``assistant`` →
    400). ``tool`` / ``function`` *result text* is not scanned for image
    markers (a ``cat`` of this file must not 400); structured images in
    ``tool`` messages are passed to the multimodal encoder.
...
ENC_ROLE_TOOL_IMAGE_MARK = "# [vision-exp-hotfix] tool result images are multimodal"
...
def _validate_no_image_sp_tokens(msg):
    """Allow prompt/tool images while rejecting system/assistant injection."""
    ...
    if _dspark_vision_value_has_image(
        ...
        raise ValueError(
            "Images are supported in user, developer, and tool messages only: "
...
def patch_encoding_text(source: str) -> tuple[str, str]:
    if (
        ...
        and ENC_ROLE_TOOL_IMAGE_MARK in source
    ...
    if ENC_ROLE_MARK in source and (
        ENC_ROLE_PAIRED_MARK not in source
        or ENC_ROLE_TOOL_MARK not in source
        or ENC_ROLE_TOOL_IMAGE_MARK not in source
    ):
...
def main() -> int:
    if len(sys.argv) > 1 and sys.argv[1] == "--status":
        ...
        print(
            ...
            and ENC_ROLE_TOOL_IMAGE_MARK in encoding
            else "NOT APPLIED",
        )
        print(
